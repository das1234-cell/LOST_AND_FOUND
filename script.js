...
items.forEach(item => {
 ...
 const isOwner = currentUser && (currentUser.phone === item.phone); // check it'is current Owner 

 let actionButtonsHTML = '';
 if (isOwner)  { // Owner show delete and edit button
  actionButtonsHTML = ` <div class = flex gap-2">
  <button onclick="openEditModal('${item.id}')" class = "text-blue-600 hover:text-blue-800 text-xs font-bold border border-blue-200 px-3 py-2 rounded-full hover:bg-blue-50 transition flex items-center"><i class ="fa-solid fa-pen-to-square mr-1"></i>Edit
  </button> 
  <button onclick="deleteItem('${item.id}')" class = "text-red-500 hover:text-red-700 text-xs font-bold border border-red-200 px-3 py-2 rounded-full hover:big-red-50 transition flex items-center"><i class="fa-solid fa-trash"></i> Delete</button>
  </div>
  `; // edit and delete button design and function
 }


 let imageHTML = item.image ? `<div class="w-full h-40`
